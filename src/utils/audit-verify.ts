...
const AUDIT_CHAIN = path.join(process.cwd(), ".opencode", "audit-chain.jsonl");
...
export function parseArgs(args: string[]): { mode: string; json: boolean; watch: boolean; output: string | null } {
  const opts: any = { mode: "verify", json: false, watch: false, output: null };
  for (let i = 0; i < args.length; i++) {
    if (["verify", "audit", "report"].includes(args[i])) opts.mode = args[i];
...
export async function loadChain(): Promise<AuditEntry[]> {
...
export function verifyHMAC(entry: AuditEntry): boolean {
...
export async function verifyChain(): Promise<ChainReport> {
  ...
  for (let i = 0; i < chain.length; i++) {
    const e = chain[i];
    // 1. Append-only check
    if (i > 0 && e.ts < chain[i - 1].ts) report.append_violations++;
    // 2. Parent link check
    if (e.parent_hash !== prevHash) report.broken_links++;
    // 3. Signature check
    if (!verifyHMAC(e)) report.signature_failures++;
    ...
    if (e.payload_hash !== expectedPayloadHash) report.broken_links++;
...
export async function getTelemetryMetrics(): Promise<{ totalCost: number; totalTokens: number; verificationGates: number; healingAttempts: number; swarmHealth: Record<string, number> }> {
  const telemetryRaw = await fs.readFile(TELEMETRY_LOG, "utf-8").catch(() => "");
...
export async function generateBoardReport(chainReport: ChainReport): Promise<BoardReport> {
...
export async function runAuditVerify(mode: string, json: boolean, output: string | null): Promise<void> {
...
export async function watchAuditChain(): Promise<void> {
...
export async function appendAuditEntry(entry: Omit<AuditEntry, "parent_hash" | "payload_hash" | "signature">): Promise<AuditEntry> {
  const chain = await loadChain();
  ...
  await fs.mkdir(path.dirname(AUDIT_CHAIN), { recursive: true });
