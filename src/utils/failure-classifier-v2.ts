...
export interface FailureAnalysis {
  ...
  lsp_diagnostics?: { file: string; line: number; message: string }[];
}

const PATTERNS: { regex: RegExp; type: FailureType; root: RootCause; hint: string }[] = [
...
export function classifyFailureV2(errorOutput: string, exitCode?: number, lspOutput?: string): FailureAnalysis {
  const snippet = errorOutput.slice(0, 600);
  const matches: { type: FailureType; root: RootCause; hint: string; score: number }[] = [];

  for (const p of PATTERNS) {
    const count = (snippet.match(p.regex) || []).length;
    if (count > 0) matches.push({ type: p.type, root: p.root, hint: p.hint, score: count * (p.type === "flaky_test" ? 1.5 : 1) });
  }

  // LSP diagnostic parsing
  const lspDiagnostics: { file: string; line: number; message: string }[] = [];
  if (lspOutput) {
