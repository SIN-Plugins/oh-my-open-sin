...
const INTENT_WEIGHTS: Record<string, { category: IntentCategory; weight: number }> = {
...
export function classifyIntent(description: string): { category: IntentCategory; score: number; matched_terms: string[] } {
  ...
  if (lower.includes("auth") && lower.includes("system")) scores.security += 2;
  if (lower.includes("state") && lower.includes("management")) scores.frontend += 2;
  if (lower.includes("query") && lower.includes("optimization")) scores.database += 2;

  const best = Object.entries(scores).sort((a, b) => b[1] - a[1])[0];
  const category = best[1] > 0 ? best[0] as IntentCategory : "unknown";
  
  structuredLog("info", "intent_classified", { description: description.slice(0, 80), category, score: best[1], matched_terms: matched });
