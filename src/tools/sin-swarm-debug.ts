...
function parseArgs(args: string[]) {
  const opts: any = { json: false, watch: false, session: null, mode: "all" };
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--json") opts.json = true;
    if (args[i] === "--watch") opts.watch = true;
    if (args[i] === "--session" && args[i+1]) opts.session = args[++i];
    if (["blackboard", "votes", "health", "audit", "all"].includes(args[i])) opts.mode = args[i];
  }
  return opts;
}
...
async function readTelemetry(filter?: string) {
...
function verifyHMAC(from: string, to: string, type: string, payload: any, ts: number, signature: string) {
...
async function render(opts: any) {
  console.clear();
  ...
  if (opts.mode === "all" || opts.mode === "blackboard") {
    ...
    if (opts.json) { console.log(JSON.stringify({ blackboard: bb }, null, 2)); return; }
  ...
  if (opts.mode === "all" || opts.mode === "health") {
    ...
    if (opts.json) { console.log(JSON.stringify({ health_matrix: hm }, null, 2)); return; }
  ...
  if (opts.mode === "all" || opts.mode === "votes") {
    ...
    if (opts.json) { console.log(JSON.stringify({ votes }, null, 2)); return; }
  ...
  if (opts.mode === "all" || opts.mode === "audit") {
    ...
    if (opts.json) { console.log(JSON.stringify({ audit }, null, 2)); return; }
...
async function main() {
  ...
  if (opts.watch) {
    ...
    while (true) { await render(opts); await new Promise(r => setTimeout(r, 2000)); }
  } else {
    await render(opts);
  }
}
