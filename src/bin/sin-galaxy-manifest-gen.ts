...
async function evolveManifest() {
  const base: any = await loadJSON(MANIFEST_PATH, {});
  ...
  if (!base.supernova_triggers) base.supernova_triggers = { budget_exhaustion_pct: 70, error_rate_spike: 0.3 };
  if (!base.consensus_engine) base.consensus_engine = { min_confidence_score: 0.85 };
  if (!base.cluster_topology) base.cluster_topology = { domains: { backend: { max_concurrency: 5 }, frontend: { max_concurrency: 5 } } };
