...
exports.GitPolicyEnforcer = void 0;
...
class GitPolicyEnforcer {
    worktreePath;
    config;
    constructor(worktreePath, config) {
    ...
    async validateBranchProtection() {
        ...
        const warnings = [];
        try {
            const { stdout: currentBranch } = await (0, exec_1.execAsync)('git rev-parse --abbrev-ref HEAD', { cwd: this.worktreePath });
    ...
    async validateCommitMessages() {
        ...
        try {
            ...
            const { stdout: status } = await (0, exec_1.execAsync)('git status --porcelain', { cwd: this.worktreePath });
            if (status.trim()) {
    ...
    async validateFiles() {
        ...
        const warnings = [];
        try {
            const { stdout: stagedFiles } = await (0, exec_1.execAsync)('git diff --cached --name-only', { cwd: this.worktreePath });
    ...
    async validateTests() {
        ...
        try {
            ...
            const { stdout: changedFiles } = await (0, exec_1.execAsync)('git diff --cached --name-only', { cwd: this.worktreePath });
            ...
            try {
                await (0, exec_1.execAsync)('npm test -- --passWithNoTests', {
    ...
    async validateLinting() {
        ...
        try {
            ...
            await (0, exec_1.execAsync)('npm run lint', {
                cwd: this.worktreePath,
                timeout: 30000
            });
        }
...
"
`;
        await fs.promises.writeFile(`${hooksDir}/pre-commit`, preCommitHook, { mode: 0o755 });
    ...
    async generateReport() {
        ...
        if (result.warnings.length > 0) {
            ...
            for (const w of result.warnings) {
                ...
                if (w.suggestion)
                    report += `  Suggestion: ${w.suggestion}\n`;
            }
            report += '\n';
        }
...
exports.GitPolicyEnforcer = GitPolicyEnforcer;
//# sourceMappingURL=GitPolicyEnforcer.js.map